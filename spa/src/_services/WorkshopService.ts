...
const WorkshopService = {
  ...
  async create(workshop: FormData) { // FormData pour upload images
    return Axios.post("/api/admin/workshops", workshop, {
      headers: { "Content-Type": "multipart/form-data" }
    });
  },

  async update(id: number, workshop: FormData) { // idem pour update
    // Laravel accepte PUT via POST avec _method=PUT
    return Axios.post(`/api/admin/workshops/${id}?_method=PUT`, workshop, {
      headers: { "Content-Type": "multipart/form-data" }
    });
  },
